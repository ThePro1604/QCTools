...
try:
    os.remove("C:/QCCenter/Logs/JsonFaceLiveness2Excel.log")
    logging.basicConfig(filename='C:/QCCenter/Logs/JsonFaceLiveness2Excel.log')
except FileNotFoundError:
    logging.basicConfig(filename='C:/QCCenter/Logs/JsonFaceLiveness2Excel.log')


def ToolRun_JsonFaceLiveness2Excel():
    ...
    def JsonFaceLiveness2Excel(folder, filetype):
        data_list = []
        ...
        def create_list_from_json(XMLfile):
            ...
            if os.path.getsize(XMLfile) != 0:
                ...
                data = text_file.read()
                text_file.close()
                faceliveness_verification = data[data.rfind("PageAsSeparateDocumentProcessingReports"):data.rfind("PersonalInfoValidationReport")]
                # data_list.append(XMLfile[XMLfile.rfind("\\") + 1:XMLfile.index(".")])
                # print(faceliveness_verification)
                ...
                if "PassiveFaceLivenessDetectionReport" in faceliveness_verification:
                    temp = faceliveness_verification[faceliveness_verification.find("PassiveFaceLivenessDetectionReport"):faceliveness_verification.rfind(",")]
                    data_list.append(temp[temp.find(": ")+2:])
                else:
                    data_list.append("")
        ...
        popup = sg.Window("Complete", layout_popup, icon=r'N:\Images\Shahaf\Projects\Assests\JsonFaceLiveness2Excel.ico')
    ...
    description = [
        [sg.Text("Extracts the PassiveFaceReportResult from json files to excel ")],
    ]
    ...
    window = sg.Window("JsonFaceLiveness2Excel", layout, icon=r'N:\Images\Shahaf\Projects\Assests\JsonFaceLiveness2Excel.ico', finalize=True)
